...
def main():
    ...
    for x, kind, title, desc, label, url in [
        (M, "doc", "Ваші документи", "Рахунки, накладні та платежі з нашої облікової системи. Переглядайте склад документа й повторюйте потрібні товари у новому замовленні.", "Переглянути документи →", "/my-account/folio-documents/"),
    ...
    p("Після успішної обробки документи автоматично зберігаються в нашій обліковій системі, а наявний товар резервується під ваше замовлення. Підсумок видно в кабінеті.", 50, 566, CW-32, 11, white, maxh=47)
    ...
    p("Зареєструйтеся та зверніться до нас: підключимо оптовий доступ,<br/>перегляд ваших документів і балансу в кабінеті.", M, 731, 421, 9.5, MUTED)
