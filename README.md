...
<summary><strong>1) mu-plugins/psu-force-per-page.php — авто-пересчёт per_page</strong></summary>
    ...
    **Идея:** количество товаров на странице = **колонки × ряды**.  
    Колонки измеряются на клиенте (по CSS Grid), сохраняются в cookie; сервер подставляет `posts_per_page`.
    ...
    **Константы (настройки):**
    ...
    | `PSUFP_ROWS_DESKTOP` | рядов на десктопе (>480px) | `3` |
    | `PSUFP_ROWS_MOBILE` | рядов на обычных мобилках (321–480px) | `3` |
    | `PSUFP_ROWS_XSMALL` | рядов на узких телефонах (≤320px) | `2` |
    | `PSUFP_FALLBACK_COLS` | кол-во колонок, если cookie ещё нет | `5` |
    | `PSUFP_DEBUG` | включить отладку (плашка + console.log) | `false` |

    **Как это работает:**
    - *CSS* (в теме) задаёт сетку: `grid-template-columns: repeat(auto-fit, minmax(...))` — это и определяет фактическое число колонок.
    - *JS* один раз меряет колонки и ширину экрана, кладёт `psu_cols` и `psu_rows` в cookie.
    - *PHP* берёт их и выдаёт `per_page = cols × rows`.
    - При ресайзе/смене ориентации JS обновляет cookie и **ровно один раз** перезагружает страницу, чтобы перегенерить выдачу.

    **Где перехватываем per_page:**
    - `loop_shop_per_page` — стандартный фильтр Woo.
    - `pre_get_posts` (главный запрос, архивы товаров).
    - `woocommerce_product_query` (только для контекста архивов товаров).

    **FAQ / Трюки:**
    - Нужен быстрый оверрайд? Добавь к URL `?per_page=12` — модуль уважит параметр.
    - Видно меньше/больше товаров, чем ожидалось? Проверь:
      1) сколько колонок реально считает CSS (посмотри наш отладочный блок),  
      2) что `psu_rows` соответствует брейкпоинту,  
      3) что cookie обновились после ресайза (происходит 1 раз).
    - Хочешь другое правило рядов для мобилок? Поменяй `PSUFP_ROWS_*`.  
