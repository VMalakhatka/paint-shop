<?php
/*
Plugin Name: PC Wholesale Quick Order
Description: Табличный «быстрый заказ» для оптовиков + массовое добавление в корзину.
Version: 1.0.0
Author: PaintCore
*/
if (!defined('ABSPATH')) exit;

/**
 * Шорткод [pc_quick_order cat="slug" per_page="50" show_stock="1"]
 * Если cat не задан — берёт текущую product_cat (когда шорткод в архиве категории).
 */
add_shortcode('pc_quick_order', function($atts){
    if (!function_exists('wc_get_product')) return '';

    // 1) Атрибуты
    $a = shortcode_atts([
        'cat'        => '',
        'per_page'   => 50,
        'show_stock' => '1',
        'role_only'  => '',
        'orderby'    => 'sku', // title|sku|date
        'order'      => 'ASC',
    ], $atts, 'pc_quick_order');

    // переопределяем атрибуты из GET
    if (isset($_GET['orderby'])) {
        $g = strtolower(sanitize_text_field($_GET['orderby']));
        if (in_array($g, ['title','sku','date','price'], true)) {
            $a['orderby'] = $g;
        }
    }
    if (isset($_GET['order'])) {
        $ord = strtoupper(sanitize_text_field($_GET['order'])) === 'DESC' ? 'DESC' : 'ASC';
        $a['order'] = $ord;
    }

    // (опционально) ограничить по ролям
    if ($a['role_only'] !== '') {
        $need = array_map('trim', explode(',', $a['role_only']));
        $u = wp_get_current_user();
        if (!$u || empty($u->roles) || count(array_intersect($need, $u->roles)) === 0) {
            return '<p>Недостаточно прав для быстрого заказа.</p>';
        }
    }

    // 2) Категория: из ?cat=..., из атрибута, либо из контекста архива
    $cat_slugs = [];
    if (!empty($_GET['cat'])) {
        $cat_slugs = array_filter(array_map('sanitize_title', preg_split('/[,\s]+/', (string)$_GET['cat'])));
    }
    if (!$cat_slugs && $a['cat'] !== '') {
        $cat_slugs = array_filter(array_map('sanitize_title', preg_split('/[,\s]+/', (string)$a['cat'])));
    }
    if (!$cat_slugs && is_product_category()) {
        $term = get_queried_object();
        if ($term && !is_wp_error($term)) $cat_slugs = [$term->slug];
    }

    // 3) Пагинация и сортировка
    $paged = isset($_GET['qo_page']) ? max(1, (int)$_GET['qo_page']) : 1;
    $order = (strtoupper($a['order']) === 'DESC') ? 'DESC' : 'ASC';

    $args = [
        'post_type'      => 'product',
        'post_status'    => 'publish',
        'posts_per_page' => max(1, (int)$a['per_page']),
        'paged'          => $paged,
        'order'          => $order,
    ];

    switch (strtolower($a['orderby'])) {
    case 'sku':
        $args['meta_key']  = '_sku';
        $args['orderby']   = 'meta_value';
        $args['meta_type'] = 'CHAR';
        break;
    case 'price':
        $args['meta_key']       = '_price';
        $args['orderby']        = 'meta_value_num';
        $args['ignore_sticky_posts'] = true;
        break;
    case 'date':
        $args['orderby']  = 'date';
        break;
    default:
        $args['orderby']  = 'title';
    }

    if ($cat_slugs) {
        $args['tax_query'][] = [
            'taxonomy'         => 'product_cat',
            'field'            => 'slug',
            'terms'            => $cat_slugs,
            'operator'         => 'IN',
            'include_children' => true,
        ];
    }

    $q = new WP_Query($args);
    $build_pager = function(WP_Query $q, int $paged): string {
    if ((int)$q->max_num_pages <= 1) return '';
    $base = remove_query_arg('qo_page');
    $out  = '<nav class="pc-qo-pager" style="margin:12px 0;display:flex;gap:8px;flex-wrap:wrap">';
    for ($i = 1; $i <= (int)$q->max_num_pages; $i++) {
        $link = add_query_arg('qo_page', $i, $base);
        $cur  = ($i === $paged) ? ' style="font-weight:700;text-decoration:underline"' : '';
        $out .= '<a href="'.esc_url($link).'"'.$cur.'>'.esc_html($i).'</a> ';
    }
    $out .= '</nav>';
    return $out;
    };
    $pager_html = $build_pager($q, $paged);
    if (!$q->have_posts()) return '<p>Товары не найдены.</p>';

    // 4) Начинаем буфер ТОЛЬКО теперь
    $nonce = wp_create_nonce('pc_bulk_add');
    ob_start(); ?>

    <div class="pc-qo-wrap">
      <div class="pc-qo-toolbar" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
        <button type="button" class="button button-primary pc-qo-addall" data-nonce="<?php echo esc_attr($nonce); ?>">
          Добавить всё в корзину
        </button>

        <!-- Переключатели сортировки (по желанию) -->
        <?php
            $base = remove_query_arg('qo_page');

            $mk = function($key, $order = null) use ($base, $a) {
            return esc_url(add_query_arg([
                'orderby' => $key,
                'order'   => $order ?: $a['order'],
                'qo_page' => 1,
            ], $base));
            };

            $bold = function($key, $label) use ($a, $mk) {
            $url   = $mk($key);
            $isCur = ($a['orderby'] === $key);
            $style = $isCur ? ' style="font-weight:700;text-decoration:underline"' : '';
            return '<a href="'.$url.'"'.$style.'>'.$label.'</a>';
            };

            // Кнопка-переключатель направления
            $nextOrder = ($a['order'] === 'ASC') ? 'DESC' : 'ASC';

            // Для цены — обратная надпись (DESC → «ЗМЕН», ASC → «ЗБІЛ»)
            // Для остальных — как было раньше
            $label     = ($nextOrder === 'DESC') ? 'ЗБІЛ' : 'ЗМЕН';
            $toggleUrl = $mk($a['orderby'], $nextOrder);
            ?>
            <span style="opacity:.7">Сортувати:</span>
            <?= $bold('title','по назві'); ?> ·
            <?= $bold('sku','по SKU'); ?> ·
            <?= $bold('price','по ціні'); ?>
            <a href="<?= $toggleUrl; ?>" style="margin-left:8px">⇅ <?= esc_html($label); ?></a>

        <span class="pc-qo-message" style="margin-left:auto;"></span>
      </div>
      <?php echo $pager_html; ?>

      <div class="pc-qo-table-wrap">
        <table class="pc-qo-table">
            <thead>
            <tr>
                <th style="width:44%;">Товар</th>
                <th>SKU</th>
                <th>Ціна</th>
                <?php if ($a['show_stock'] === '1'): ?>
                <th>У наявності</th>
                <th>У кошику</th>
                <th>Доступно</th>
                <?php endif; ?>
                <<th class="pc-qo-qty">К-сть</th>
            </tr>
            </thead>
          <tbody>
          <?php
          
          $cart_qty_map = ( WC()->cart ) ? WC()->cart->get_cart_item_quantities() : [];
          while ($q->have_posts()): $q->the_post();
                $product = wc_get_product(get_the_ID());
                if (!$product || !$product->is_purchasable() || $product->is_type('variable')) continue;

                $pid        = $product->get_id();
                $price_html = $product->get_price_html();
                $sku        = $product->get_sku();

                // 1) Скільки всього на складі (якщо порожньо або <0 — вважаємо 0)
                $stock_raw = get_post_meta($pid, '_stock', true);
                $stock_qty = wc_stock_amount($stock_raw);
                if ($stock_qty === '' || $stock_qty < 0) $stock_qty = 0;

                // 2) Скільки вже в кошику
                $in_cart = isset($cart_qty_map[$pid]) ? (int) $cart_qty_map[$pid] : 0;

                // 3) «Доступно» РІВНО за формулою: на складі − у кошику (не нижче 0)
                $available_display = max(0, (int)$stock_qty - (int)$in_cart);

                // 4) Скільки дозволити додати
                $available_for_add = $available_display;

                // якщо у товару є персональний максимум (і він >0) — врахуємо його
                $max_by_product = (int) $product->get_max_purchase_quantity();
                if ($max_by_product > 0) {
                    $available_for_add = min($available_for_add, $max_by_product);
                }

                // крок/дизейбл інпуту
                $step     = max(1, (int) $product->get_min_purchase_quantity());
                $disabled = ($available_for_add <= 0) ? 'disabled' : '';
                $placeholder = ($available_for_add > 0) ? '0' : '0';
                ?>
                <tr class="pc-qo-row"
                    data-id="<?php echo esc_attr($pid); ?>"
                    data-available="<?php echo esc_attr($available_for_add); ?>">
                <td class="pc-qo-title"><a href="<?php the_permalink(); ?>" target="_blank" rel="noopener"><?php the_title(); ?></a></td>
                <td class="pc-qo-sku"><?php echo esc_html($sku ?: '—'); ?></td>
                <td class="pc-qo-price"><?php echo $price_html ?: '—'; ?></td>

                <td class="pc-qo-stock"><?php echo esc_html($stock_qty); ?></td>
                <td class="pc-qo-incart"><?php echo esc_html($in_cart); ?></td>
                <td class="pc-qo-left"><?php echo esc_html($available_display); ?></td>

                <td class="pc-qo-qty">
                <input type="number"
                        min="0"
                        step="<?php echo esc_attr($step); ?>"
                        max="<?php echo esc_attr($available_for_add); ?>"
                        class="pc-qo-input"
                        <?php echo $disabled; ?>
                        placeholder="0">
                </td>
                </tr>
                <?php
            endwhile; wp_reset_postdata(); ?>
          </tbody>
        </table>
      </div>

     <?php echo $pager_html; ?>  <!-- пагинация СНИЗУ -->
    </div>
    <?php
    return ob_get_clean();
});

/**
 * AJAX: массовое добавление
 * data: items = [{id:123, qty:2}, ...]
 */
add_action('wp_ajax_pc_bulk_add_to_cart',     'pc_qo_bulk_add');
add_action('wp_ajax_nopriv_pc_bulk_add_to_cart', 'pc_qo_bulk_add');
function pc_qo_bulk_add(){
    check_ajax_referer('pc_bulk_add');

    if (empty($_POST['items']) || !is_array($_POST['items'])) {
        wp_send_json_error(['msg'=>'Пусто']);
    }
    $added = 0; $errors = [];

    foreach ($_POST['items'] as $row) {
        $pid = isset($row['id'])  ? (int)$row['id']  : 0;
        $qty = isset($row['qty']) ? (float)$row['qty'] : 0;
        if ($pid <= 0 || $qty <= 0) continue;

        $product = wc_get_product($pid);
        if (!$product || !$product->is_purchasable() || $product->is_type('variable')) {
            $errors[] = $pid; continue;
        }

        // ограничение по доступному количеству (грубая проверка)
        $stock = (float) get_post_meta($pid, '_stock', true);
        if ($product->managing_stock() && $stock >= 0 && $qty > $stock) {
            $qty = max(0, $stock);
        }
        if ($qty <= 0) continue;

        $res = WC()->cart->add_to_cart($pid, $qty);
        if ($res) $added++;
    }

    wc_clear_notices();
    wp_send_json_success(['added'=>$added]);
}

// На странице, где вставлен шорткод [pc_quick_order], переписываем ссылки категорий на ?cat=<slug>
add_action('wp', function () {
    if (!is_page()) return;
    $post = get_post();
    if (!$post) return;
    if (!has_shortcode($post->post_content, 'pc_quick_order')) return;

    // Все ссылки product_cat указываем на текущую страницу + ?cat=...
    add_filter('term_link', function ($url, $term, $taxonomy) use ($post) {
        if ($taxonomy !== 'product_cat') return $url;
        return add_query_arg('cat', $term->slug, get_permalink($post));
    }, 10, 3);
});

/** Стили и JS */
add_action('wp_enqueue_scripts', function(){
    // Стили
    $css = '
        .pc-qo-wrap{font-size:12px}
        .pc-qo-table{width:100%; border-collapse:collapse; table-layout:fixed}
        .pc-qo-table th,.pc-qo-table td{
            padding:6px 8px;
            border-bottom:1px solid #eee;
            vertical-align:middle;
            line-height:1.2;
            font-size:12px;
        }
        .pc-qo-table thead th{background:#fafafa; font-weight:600}
        .pc-qo-title a{
            color:inherit;
            text-decoration:none;
            display:inline-block;
            max-width:100%;
            overflow:hidden;
            text-overflow:ellipsis;
            white-space:nowrap;
        }

        /* SKU */
        .pc-qo-sku{width:110px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis}

        /* Цена */
        .pc-qo-price{width:90px; text-align:right; white-space:nowrap}

        /* Остатки */
        .pc-qo-stock,.pc-qo-incart,.pc-qo-left{
            width:70px;
            text-align:right;
            white-space:nowrap
        }

        /* Колонка количество */
        .pc-qo-table th.pc-qo-qty,
        .pc-qo-table td.pc-qo-qty{
            width:56px;
            max-width:56px;
            text-align:right;
            overflow:hidden;
            white-space:nowrap;
        }

        /* Сам input */
        .pc-qo-qty .pc-qo-input{
            width:100%;
            box-sizing:border-box;
            text-align:right;
            font-size:11px;
            padding:1px 4px;
        }

        /* убираем стрелки у number */
        .pc-qo-qty .pc-qo-input::-webkit-outer-spin-button,
        .pc-qo-qty .pc-qo-input::-webkit-inner-spin-button{
            -webkit-appearance:none;
            margin:0;
        }
        .pc-qo-qty .pc-qo-input{ -moz-appearance:textfield; }

        .pc-qo-toolbar{margin:6px 0 10px; font-size:12px}
        .pc-qo-pager{margin:10px 0; display:flex; gap:6px; flex-wrap:wrap}
        .pc-qo-sort{display:flex; gap:6px; align-items:center; flex-wrap:wrap; margin-left:8px}
        .pc-qo-sort > span{opacity:.7; margin-right:2px}
        .pc-qo-sort .btn{display:inline-block; padding:3px 8px; border:1px solid #ddd; border-radius:6px; text-decoration:none}
        .pc-qo-sort .btn.is-active{border-color:#111; font-weight:700}
        .pc-qo-sort .btn.toggle{margin-left:6px}
    ';

    wp_register_style('pc-qo-inline', false);
    wp_enqueue_style('pc-qo-inline');
    wp_add_inline_style('pc-qo-inline', $css);
    // JS
    wp_add_inline_script('jquery', "
        jQuery(function($){

        // Клампимо значення інпутів локально (не даємо ввести більше, ніж доступно)
        $(document).on('input change', '.pc-qo-input', function(){
            var \$row = $(this).closest('.pc-qo-row');
            var available = parseFloat(\$row.data('available')) || 0;
            var step = parseFloat($(this).attr('step')) || 1;
            var v = parseFloat($(this).val() || 0);
            if (v < 0) v = 0;
            if (available >= 0 && v > available) v = available;
            // підганяємо під крок
            if (step > 0) v = Math.floor(v/step)*step;
            $(this).val(v || '');
        });

        $(document).on('click','.pc-qo-addall',function(){
            var \$btn = $(this), nonce = \$btn.data('nonce');
            var items = [];
            $('.pc-qo-row').each(function(){
            var id = parseInt($(this).data('id'),10);
            var available = parseFloat($(this).data('available')) || 0;
            var qty = parseFloat($(this).find('.pc-qo-input').val()||0);
            if(id>0 && qty>0){
                if (available >= 0 && qty > available) qty = available; // подвійний захист
                if (qty>0) items.push({id:id, qty:qty});
            }
            });
            if(!items.length){ $('.pc-qo-message').text('Немає вибраних кількостей.'); return; }
            \$btn.prop('disabled',true);
            $.post('".admin_url('admin-ajax.php')."', {action:'pc_bulk_add_to_cart', _ajax_nonce:nonce, items:items}, function(resp){
            if(resp && resp.success){
                $('.pc-qo-message').text('Додано позицій: '+resp.data.added);
                // Оновимо сторінку/фрагменти кошика, щоб «У кошику» та «Доступно» стали актуальними
                $(document.body).trigger('wc_fragment_refresh');
                // За потреби можна зробити повний reload, щоб перерахувати ліміти:
                // location.reload();
            }else{
                $('.pc-qo-message').text('Помилка додавання.');
            }
            \$btn.prop('disabled',false);
            });
        });
        });
        ");
});