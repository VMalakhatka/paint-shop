...
(async () => {
    ...
    try {
        for (const width of [1440,1024,768,390,320]) {
            ...
            const categoryPath = new URL(await form.getAttribute('action')).pathname;
            const units = form.locator('select[name="unit"]');
            await form.locator('.psu-catalog-filters__advanced > summary').click();
            await units.waitFor();
            assert((await units.locator('option').allTextContents()).includes('80 мл'));
            // An unrelated unit exists globally but must not leak into this category's list.
            assert(!(await units.locator('option').allTextContents()).includes('1000 мл'));
            await units.selectOption({label:'80 мл'});
            const unit = await units.inputValue();
            await form.getByRole('button',{name:'Застосувати фільтри',exact:true}).click();
            await page.waitForLoadState('load');
            const url = new URL(page.url());
            assert.equal(url.pathname,categoryPath);
            ...
            await form.locator('#psu-catalog-search').fill('фарба');
            await form.getByRole('button',{name:'Застосувати фільтри',exact:true}).click();
            await page.waitForLoadState('load');
            ...
            assert(varnishOptions.includes('250 мл') && varnishOptions.includes('125 мл'));
            ...
            await units.selectOption({label:'250 мл'});
            await form.getByRole('button',{name:'Застосувати фільтри',exact:true}).click();
            await page.waitForLoadState('load');
            assert.equal(new URL(page.url()).searchParams.get('unit'),'250 мл');
            ...
            if (width===390) await form.screenshot({path:`${output}/filter-mobile-help.png`});
            await form.locator('#psu-catalog-search').fill('лак');
            await form.getByRole('button',{name:'Застосувати фільтри',exact:true}).click();
            await page.waitForLoadState('load');
            ...
            await form.locator('#psu-catalog-search').fill('KR-79406');
            await form.getByRole('button',{name:'Застосувати фільтри',exact:true}).click();
            await page.waitForLoadState('load');
