...
class Exporter {
    public static function handle(){
        ...
        $type  = sanitize_key($_GET['type'] ?? 'cart');
        $fmt   = sanitize_key($_GET['fmt']  ?? 'csv');
        ...
        if ($type === 'order') {
            ...
            if (!current_user_can('manage_woocommerce')) {
                if ((int)$order->get_user_id() !== (int)get_current_user_id()) wp_die('Forbidden', '', ['response'=>403]);
            }
    ...
    public static function extract_from_cart(array $want, string $split): array {
        ...
        foreach (WC()->cart->get_cart() as $it) {
            $p = $it['data'] ?? null; if (!$p instanceof \WC_Product) continue;
            $sku = $p->get_sku();
            ...
            // план списання — через фільтр (core може підложити), або primary
            ...
            if ($split === 'per_loc' && $plan){
                foreach ($plan as $tid=>$q){
                    ...
                    $rows[] = array_intersect_key([
                        'sku'=>$sku,'gtin'=>$gtin,'name'=>$name,
                        'qty'=>$q,'price'=>wc_format_decimal($price,2),
                ...
                $rows[] = array_intersect_key([
                    'sku'=>$sku,'gtin'=>$gtin,'name'=>$name,
                    'qty'=>$qty,'price'=>wc_format_decimal($price,2),
    ...
    public static function extract_from_order(int $order_id, array $want, string $split): array {
        $rows = []; $order = wc_get_order($order_id); if (!$order) return $rows;
        foreach ($order->get_items() as $item){
            $p = $item->get_product(); $name = html_entity_decode(wp_strip_all_tags($item->get_name()), ENT_QUOTES, 'UTF-8');
            $sku=''; if ($p){ $sku=(string)$p->get_sku();
                if (!$sku && $p->is_type('variation')) { $parent=wc_get_product($p->get_parent_id()); if ($parent) $sku=(string)$parent->get_sku(); }
            }
            $gtin = $p ? Helpers::product_gtin($p) : '';
            $qty  = (float)$item->get_quantity();
            $unit = (float)$order->get_item_subtotal($item, false, false);

            // план з мети або primary
            $plan = $item->get_meta('_pc_stock_breakdown', true);
            if (!is_array($plan)) { $try=json_decode((string)$plan, true); $plan=is_array($try)?$try:[]; }
            if (!$plan) {
            ...
            if ($split==='per_loc' && $plan){
                foreach ($plan as $tid=>$q){
                    ...
                    $rows[] = array_intersect_key([
                        'sku'=>$sku,'gtin'=>$gtin,'name'=>$name,
                        'qty'=>$q,'price'=>wc_format_decimal($unit,2),
                ...
                $rows[] = array_intersect_key([
                    'sku'=>$sku,'gtin'=>$gtin,'name'=>$name,
                    'qty'=>$qty,'price'=>wc_format_decimal($unit,2),
    ...
    // --- відправка файлів ---
    protected static function send_csv(array $header, array $rows, string $name){
        nocache_headers();
    ...
    protected static function send_xlsx(array $header, array $rows, string $name){
        // підключи автолоадер, якщо поклав vendor у wp-content/vendor або в корінь WP
        ...
        if (!class_exists('\\PhpOffice\\PhpSpreadsheet\\Spreadsheet')) {
            // fallback → CSV з ремаркою
            $L = Helpers::labels();
            $rows = array_map(function($r) use ($L){ $r['_info']=$L['xls_missing']; return $r; }, $rows);
            self::send_csv(array_merge($header, ['_info']), $rows, $name);
        }
        $s = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
        $sh = $s->getActiveSheet();
        $c=1; foreach ($header as $h) $sh->setCellValueByColumnAndRow($c++,1,$h);
        $r=2; foreach ($rows as $row){ $c=1; foreach ($row as $v){ $sh->setCellValueByColumnAndRow($c++,$r,$v);} $r++; }
        foreach (range(1,count($header)) as $i) $sh->getColumnDimensionByColumn($i)->setAutoSize(true);
        nocache_headers();
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="'.$name.'.xlsx"');
        (new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($s))->save('php://output'); exit;
    }
}