...
require __DIR__ . '/../inc/WaitlistMail.php';
define('DAY_IN_SECONDS', 86400);
use PaintCore\PCOE\WaitlistMail as Mail;
$stock = [10 => ['stock' => 2, 'quantity' => 5]];
[$state, $items] = Mail::plan([], $stock, 100000);
expect(count($items), 1, 'First partial availability notifies');
[$state, $items] = Mail::plan($state, [10 => ['stock' => 5, 'quantity' => 5]], 100001);
expect($items, [], 'Growing positive stock does not duplicate');
[$state, $items] = Mail::plan($state, [10 => ['stock' => null]], 100002);
expect($items, [], 'Unknown stock never rearms');
[$state, $items] = Mail::plan($state, [10 => ['stock' => 0]], 100003);
expect($items, [], 'Stockout does not notify');
[$state, $items] = Mail::plan($state, $stock, 100004);
expect($items, [], 'Flapping stock cooldown');
[$state, $items] = Mail::plan($state, $stock, 200000);
expect(count($items), 1, 'New availability episode after cooldown');
[$state, $items] = Mail::plan($state, $stock, 300000);
expect($items, [], 'Elapsed time alone does not resend');
echo "OK: $checks total checks including mail episode rules.\n";
