/* Small OOXML writer. Formula cells are created only by the manager layout, never from document text. */
(function (scope) {
    'use strict';
    ...
    function cell(value, ref, heading) {
        if (value && typeof value === 'object' && value.type === 'formula') {
            // Only local arithmetic/rounding generated by our layout. No links, names or commands.
            if (!/^[A-Z0-9$()+*,.\/:\-]+$/.test(value.formula) || !/^-?\d+(\.\d+)?$/.test(String(value.value))) throw new Error('Invalid report formula');
            return `<c r="${ref}" s="${value.money ? 2 : 3}"><f>${escapeXml(value.formula)}</f><v>${value.value}</v></c>`;
        }
    ...
    function build(sheets) {
        ...
        files.push(['xl/workbook.xml', `<workbook xmlns="${ns}" xmlns:r="${relns}"><sheets>${sheets.map((s, i) => `<sheet name="${escapeXml(s.name)}" sheetId="${i + 1}" r:id="rId${i + 1}"/>`).join('')}</sheets><calcPr calcId="191029" fullCalcOnLoad="1"/></workbook>`]);
        ...
        sheets.forEach((s, i) => {
            ...
            const rows = s.rows.map((row, n) => {
                ...
                const height = Math.min(409, Math.max(headings.has(n + 1) ? 42 : (s.rowHeight || 36), 15 * lines + 8));
