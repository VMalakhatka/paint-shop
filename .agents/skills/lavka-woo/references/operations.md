...
aggregate v2 + sources, context/generation/configuration guards и оба экспорта.
До деплоя обновлённой Java действует legacy guard, список не подменяется складом 9.
Для одного получателя подтверждённый транзит подставляется автоматически,
для нескольких распределяется менеджером. Требуются совпадение источника, поколение
и подтверждённое происхождение; ручное число не обходит неизвестный источник.
