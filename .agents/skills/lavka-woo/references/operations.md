...
Java schema 7 классифицирует текущие учитываемые счета `*ПЕРЕМЕЩЕНИЕ` отдельно
от продаж; WordPress применяет точные маршруты sourceWarehouseId + sourceInfo,
сохранённые в сценарии. Внутри группы резерв возвращается один раз, между группами
учитывается только во входящем запасе получателя. Клиентские резервы остаются вычтенными.
Не заменять неизвестного получателя нулём и не суммировать тот же счёт с ручным
транзитом/поступлениями. Поправка не доказывает общую согласованность сети: финальная
заявка требует отдельной сверки поколений/перемещений. Источники: purchase-planning-model.php,
Java FolioProductMovementClassifier, docs/api/FOLIO_PRODUCT_ANALYTICS_API.md.
Порядок включения и диагностика: docs/OPERATIONS_RUNBOOK.md, раздел «Активные счета
на внутреннее перемещение». Нужны deploy Java+сайта и новые снимки schema 7.
