...
- После финального snapshot блок «Звіт за станами знімка» раскрывает реальные товары `NEW`, `DIRTY`, `FAILED` и `REMOVED` с пагинацией и отдельным CSV для выбранного состояния. Для каждого товара показываются причина состояния, ошибка, период движений, последнее наблюдение/применение и последняя зафиксированная смена состояния.
...
- Текущий WordPress-экран читает один активный склад. Он поддерживает
  множественный include/exclude-фильтр `current_supplier` и именованные наборы
  текущих фильтров в user meta WordPress.
- Несколько складов в одном отчёте, группы/подгруппы и отборы движений не
  рассчитывать в PHP из готовых метрик. Java должна опубликовать стабильные
