...
JS/экспорт не усредняют проценты и не превращают non-MEASURED в числа.
Нужны три capability gates и schema>=5; внедрение и операторская сверка описаны
в `docs/OPERATIONS_RUNBOOK.md`, раздел schema v5 (2026-09-06).
Расчёт закупки остаётся без автоматической поправки на потерянный спрос.

## Предварительный заказ поставщику

Владелец: `lavka-price-sync/inc/purchase-planning*.php`, страница
`lps-purchase-planning`. Расширение `purchasePlanning.version=1` хранится в JSON
существующего сценария с общей optimistic version/revision history; состав групп
разрешается server-side из глобальной карты. Java query остаётся источником метрик,
PHP считает только предварительную потребность/закупку по параметрам менеджера.
Уточнение владельца 2026-09-13: «Одесса распродажа» не участвует в пополнении
KREUL. Не возвращай склад распродажи в группу ради устранения неполных данных.
Изменение глобальной группы влияет также на отображение остатков и требует
нового preview; проверенные настройки и rollback — в `docs/OPERATIONS_RUNBOOK.md`.
Уточнение владельца 2026-09-12: любой MIN>0, включая 1 и 0,001, полностью
добавляется к прогнозу. Java отдаёт `FORECAST_PLUS_MINIMUM_STOCK` и точный
`reserveAboveForecast`; MIN=1 больше не означает прогноз без добавки. PHP не
переинтерпретирует MIN. Крупные разовые продажи KR-49600 обосновывают ручную
корректировку владельца, а не автоматическое включение разового спроса в прогноз.
Глобальные транспортные склады: `lavka-sync/inc/warehouse-map.php`, option
`lavka_transit_warehouse_ids`, helper `lavka_get_transit_warehouse_ids()`.
Отсутствующая option наследует legacy `[9]`, явный `[]` отключает вычитание.
Контракт Java `FOLIO_TRANSIT_WAREHOUSES_FRONTEND.md` подключён 2026-09-06:
capability-gated calculation.transit (до 16 ID + SHA-256 массива) в capabilities/query,
aggregate v3 + sources, context/generation/configuration guards и оба экспорта.
До деплоя обновлённой Java действует legacy guard, список не подменяется складом 9.
Sources показывают физический доступный остаток независимо от происхождения и
отдельно поставщицкую часть; это не два слагаемых. Текущая Java для непустого списка
не подтверждает согласованность независимых snapshots, поэтому общий сетевой итог
не вычитается из закупки. Ручное число не обходит `networkPlanningReady=false`.
Смена настройки инвалидирует preview/export; пересечение транспорта с получателем
запрещено. Транзит уменьшает закупку, но не спрос. ETA/назначение и отсутствие дублей
в других поступлениях подтверждает менеджер (`receiptsReviewed` по каждой группе).
Java не заявляет автоматическую сверку партий; без подтверждения preview не готов.
Группы не пересекаются; транзит не вычитается дважды; неизвестные входные данные
не заменяются нулём; политики Java запрещают закупку независимо от ручного ввода.
Документы/резервы не создаются. Preview и правки пользователя временные (2 часа),
для сохранения есть CSV/XLSX. Формулы и приёмка: `docs/OPERATIONS_RUNBOOK.md`.
Экранные фильтры preview работают над уже загруженными строками: точный
`dimensions.minimumStock=0`, подтверждённое отсутствие регулярных продаж,
`groupLevel1`, узлы `groupLevel2..6`, предварительная потребность и готовая
рекомендация. Неизвестные MIN/продажи не приравниваются к нулю. Фильтры не меняют
сессию расчёта и полный CSV/XLSX export. До готовой заявки остаются согласованный
сетевой срез либо сверка перемещений, массовое подтверждение ожидаемых поступлений
и долговечный черновик заказа. Черновик должен переживать обновление страницы,
хранить сценарий/период/поколения снимков и ручные правки, а при изменении источника
становиться явно устаревшим без автоматического затирания корректировок. Его
сохранение не создаёт заказ, документ ФОЛІО или резерв (план уточнён 2026-09-12).
...
- Для порции отдельно показывай `processedSku`, подтверждённые `committedChunks` и разницу как skipped. Пропущенный SKU с `committed=false` не изменён в ФОЛІО и не получает `VERIFIED`; это не превращает безопасно завершённую порцию в ошибку склада.
- `ACCOUNTING_PRICE_DIVIDE_BY_ZERO` безопасно продолжает очередь только при структурированной диагностике Java с `skipped=true`, `committed=false` и `rollbackConfirmed=true`. Показывай базу, склад, SKU, job ID, этап, SQL `errorCode/sqlState/message`, подтверждение rollback, рекомендацию и кандидатов документов. `formulaConfirmed=false` и `documentConfirmed=false` означают, что точная формула и причинный документ не доказаны.
- Обычный `FAILED` завершает текущий склад, строит финальный snapshot и позволяет перейти к следующему складу.
- `FAILED_PARTIAL` и `OUTCOME_UNKNOWN` останавливают всю кампанию и расписание до ручной проверки. Исключение разрешено только для точного верхнеуровневого Java-кода `FOLIO_LOCK_BUSY_RETRYABLE_AFTER_SNAPSHOT`, который гарантирует rollback текущего SKU и проверку всех более ранних commit. Тогда WordPress учитывает подтверждённый прогресс, ждёт 10 минут, при наличии минимум 25 минут окна назначает retry, после ожидания проверяет резерв 15 минут и строит свежий snapshot перед выбором оставшихся SKU. Старый пакет не повторять. При недостатке времени выполнить финальный snapshot и `PAUSED_TIME_LIMIT`; недельное расписание не отключать. Не распознавать это состояние по тексту исключения или SQL. Точный контракт: `docs/api/FOLIO_ACCOUNTING_PRICE_LOCK_RETRY_BACKEND_TASK.md`.
- В старом ответе `FAILED` без `failedChunk` поля `currentArt`/`checkpointArt` являются только последним известным контекстом. Их нельзя подписывать как виновный или отклонённый SKU.
...
- Исчерпание бюджета нового пакета сохраняется как `stop_reason=time_limit`: после финального снимка останавливается вся очередь, даже если wall-clock deadline ещё не наступил. История склада хранит `PARTIAL_TIME_LIMIT`/`NOT_PROCESSED_TIME_LIMIT` и `remaining_skus`; очередное недельное расписание не отключается. Источник: `accounting-price-campaign.php`, регламент: `docs/OPERATIONS_RUNBOOK.md`.
...
- После Java/Flyway V14 постоянный журнал `folio_accounting_price_diagnostic` доступен из строки склада независимо от новой кампании. Обязательны фильтры `source_database + warehouse_id`; дополнительные точные фильтры — SKU, job ID, preview/apply и диапазон дат. Интерфейс и CSV читают записи по `id DESC` с keyset pagination. До V14 блок показывает понятное предупреждение, не ломая текущие campaign/snapshot отчёты.
...
- `coverageDays` — покрытие текущего запаса регулярным спросом, а не процент дней
  присутствия товара. Подневную доступность показывай только после capability
  backend. Для группы складов считай дневное объединение `ANY eligible member`,
  не среднее процентов; физическую детализацию сохраняй. `MIN_TVRZAP=0` означает,
  что склад неприменим к stockout этого SKU. Контракт:
  `docs/api/FOLIO_PRODUCT_AVAILABILITY_BACKEND_TASK.md` (зафиксировано 2026-09-03).
...
### Warehouse usage per scenario, 2026-09-13

Канон: [режим использования склада](../../../../docs/OPERATIONS_RUNBOOK.md#режим-использования-склада-в-сценарии).
`calculation.stockOnlyWarehouseIds` — положительные ID внутри scope; отсутствие
поля сохраняет FULL. STOCK_ONLY даёт только текущие количества, исключая склад из
продаж, политики MIN/MAX, упаковки/MOQ, финансов и маски наличия. Java capability
`features.warehouseUsage.version=1` и appliedFilters обязательны до принятия
результата; PHP purchase использует отдельные demandWarehouseIds. Не удалять такой
склад из stock scope и не включать в availability union. Проверено на production 2026-09-13 после деплоя обеих частей: рабочие сценарии
№3 v5 и №4 v3 используют stock-only 15. Пересчёт №4 подтвердил отсутствие склада
15 в спросе/истории и сохранил его текущие количества. Старые preview не переносить.
