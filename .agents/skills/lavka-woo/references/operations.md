...
доказывает готовность физического перемещения. `group.minimumWarehouseId` выбирает
единственный источник `reserveAboveForecast`; без поля используется receivingWarehouseId.
Не суммировать MIN участников. Источник должен входить в full-analytics часть
группы и иметь подтверждённую разрешающую политику; состав продаж/истории и MAX
не меняется. Preview version 4 отклоняет сессии со старым суммированием MIN; он временный,
...
## Внутренние резервы в закупке (2026-09-20)

Уточнение владельца: все активные учитываемые счета *ПЕРЕМЕЩЕНИЕ прибавляются
к свободному остатку склада, на котором находятся их строки, независимо от получателя.
Java schema 7 выделяет эти резервы; WordPress суммирует поправку по исходным складам
группы. Не строить маршруты и не требовать дополнительную сверку сети для этой
поправки. Клиентские резервы остаются вычтенными. SourceInfo — только подпись счёта.
Отдельный существующий контроль физического транзита не изменяется. Правило применяется
к потребности, излишкам и лимиту склада приёмки. Источники: purchase-planning-model.php,
...
Production recovery 21.09.2026: Java `ac629d9`, V16 success, обе таблицы имеют
34 колонки в одинаковом порядке; контрольный склад 1, generation 336, schema 7
успешно опубликован (`ACTIVE`), строки сверены с MariaDB. Сохранённое расписание
восстановлено на следующий разрешённый запуск, maintenance lock снят. После
восстановления runtime сначала один
контрольный snapshot под собственным общим lock, затем разрешённое включение
...
Проверено 2026-09-22 по решению владельца и коду: `canonical_stem` преобразует
украинскую и русскую кириллицу посимвольно по фиксированной таблице, включая
суффиксы; `Ж→g`, `Ц→c`, `К→k`. Имена `kcm-…` подтверждены по назначенным
production media. Сохраняется исключение `РСУ→rcy` и prefix-map extension hook.
SKU не изменяется; неизвестные символы и конфликты имён остаются блокирующими.
