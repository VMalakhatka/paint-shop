...
- Видимая панель `psu-search-filters` объединяет поиск, бренд, склады, наличие и
  цену. Точный SKU/GTIN ищется по реальным meta keys синхронизации, а Relevanssi
  должен индексировать `_sku`, `_wc_gtin_code`, `_global_unique_id` и соседние
  поддерживаемые GTIN keys.
