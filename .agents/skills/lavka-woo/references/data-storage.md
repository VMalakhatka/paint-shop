...
первичный ключ клиента и revision обеспечивают CAS. Это явные запросы клиента,
не аналитическая проекция ФОЛИО. `_pcoe_demand_event` через Woo CRUD сохраняет
исходные строки и результат обработки отслеживаемого черновика. `cart_added`
не является покупкой; ошибка/коррекция корзины не является дефицитом.
Погашение по расходным накладным и email пока не реализованы.
Точный lifecycle, ограничения и rollback:
...
Постоянные арифметические ошибки перерасчёта Java сохраняет в MariaDB-таблице
`folio_accounting_price_diagnostic` (Flyway V14). Уникальность задаётся job, базой,
складом, SKU и preview/apply; полный структурированный payload хранится в
`diagnostics_json`. WordPress только читает этот журнал подготовленными запросами
с обязательной изоляцией по базе и складу. Отсутствие таблицы до деплоя V14 должно
обрабатываться как недоступная история, а не как пустая доказанная диагностика.
Проверено на production 2026-09-07: `$wpdb` может отклонить параметризованный
запрос к этой Java-таблице с кириллическим SKU как содержащий неверные данные,
хотя MariaDB хранит значение корректно. Для точного фильтра диагностического
журнала используй `HEX(sku) = %s` с `strtoupper(bin2hex($sku))`; не применяй
`sku = %s` и не меняй исходный SKU или кодировку данных ради обхода проверки.
