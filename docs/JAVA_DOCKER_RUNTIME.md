...
## Контролируемое восстановление V16

Проверено: 2026-09-21, production. Для восстановления частично подготовленной
movement snapshot schema установлен Java `ac629d9` из чистого detached checkout,
без незавершённых локальных изменений. До сборки прошли 19 профильных тестов
на Java 17, включая отдельную одноразовую MariaDB 10.6.19. Образ собран для
`linux/amd64`; revision label и SHA256 переданного архива проверены на сервере.

Перед остановкой Java проверены отсутствие активных Java-задач и собственный
общий maintenance lock. Сделан полный application DB dump, сохранены реальные
env, mounts, network mode, restart policy, ulimits и security options старого
контейнера. Локальные `.env` не переносились, production `.env.prod` не менялся.
Дамп прошёл gzip-проверку и проверку завершающей строки, но не test restore.
Приватные env/inspect/backup-файлы остаются вне Git с ограниченным доступом.

Старый контейнер сохранён остановленным как `kreul-api-before-v16-20260921`,
старый образ — `kreul-api:before-v16-20260921`. Новый контейнер использует
`kreul-api:recovery-v16-ac629d9`; после internal health новый образ также получил
тег `kreul-api:latest`. Internal и external health вернули HTTP 200; V16 успешна,
порядок и количество колонок live/staging совпадают. Теги и имена здесь являются
исторической записью операции, а не неизменными константами следующего деплоя.

При сбое запуска возврат сохранённого контейнера восстанавливает runtime,
но **не откатывает DDL**. До повторного снимка повторно сверить Flyway и обе
таблицы; не удалять историю миграций или активные поколения. Успех runtime
не разрешает автоматически включать расписание: необходим один успешный
контрольный снимок по [операционному регламенту](OPERATIONS_RUNBOOK.md#восстановление-production-после-частичной-v16).

При диагностике через SSH stderr (включая locale warnings) нельзя объединять
с JSON stdout `docker inspect`. После ошибки подготовительного скрипта сначала
проверить фактический контейнер и сохранённые файлы, затем выбирать повтор или
rollback; не повторять замену вслепую.
