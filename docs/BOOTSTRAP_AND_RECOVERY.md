...
### WordPress deploy: отсутствие старых резервных копий

Проверено: 2026-09-10, production preview и `wp-content/deploy_safe.sh`.
Ротация должна успешно пропускать шаблон, по которому ещё нет архивов. Иначе
`set -euo pipefail` останавливает deploy после синхронизации, до очистки кэша,
из-за ненулевого результата `ls`. Обе функции ротации теперь сначала проверяют
совпадения через `compgen -G`; другие ошибки ротации не скрываются.
Перед первым запуском исправленного скрипта обновить `~/deploy_safe.sh` из
`~/deploy/paint-shop/wp-content/deploy_safe.sh` после fast-forward checkout.
`DRY_RUN=1` относится к rsync: подготовка, backup и ротация всё ещё выполняются.
...
### Витрина падает, а админка открывается

Проверено локально 2026-09-07: после расширения клиентских документов для менеджера
обработчик `pc_folio_documents_enqueue_assets` получил необязательный числовой ID.
При `wp_enqueue_scripts` WordPress передавал пустую строку и вызывал `TypeError`
на главной и в категориях. Регистрация обработчика теперь принимает ноль аргументов;
менеджерская страница передаёт ID отдельным прямым вызовом.

При таком симптоме проверяйте свежий fatal в WooCommerce logs и `wp-content/debug.log`,
а не перезапускайте БД вслепую. После исправления проверьте витрину в браузере и
`wp eval-file wp-content/plugins/pc-order-import-export/tests/frontend-document-assets.php`
на локальном сайте. Активация или изменение настроек для этого исправления не нужны.
...
## Снимок задолженности завис после деплоя Java

Проверено: 2026-09-24, production; источники: snapshot status/database-activity API,
Docker StartedAt и `FolioCustomerBalanceSnapshotService` / `FolioCustomerBalanceSnapshotDao`.

Версия до исправления heartbeat оставляет lease старого расчёта до двух часов.
Новая версия использует TTL 120 секунд и отдельное продление каждые 15 секунд;
после остановки процесса страница увидит истечение lease при очередной проверке,
watchdog проверяет восстановление каждые 30 секунд. Долгий SQL не мешает heartbeat.
Явные overrides TTL/периода recovery сохраняют заданные значения; перед деплоем
проверьте их. Уже выданная двухчасовая lease требует прежнего порядка ниже.
Перед ручным восстановлением сопоставьте время старта контейнера, heartbeat и
неизменяющийся прогресс BUILDING. Одно отсутствие активности MSSQL не доказывает
зависание: запрос мог завершиться между проверками. Проверьте отсутствие другого
живого исполнителя. Учитывайте различие часовых поясов Java, Docker и MariaDB.

При разрешённом восстановлении освободите только доказанно брошенную строку
`folio_balance_snapshot_lock`, условным UPDATE по прочитанным owner_id/updated_at
и ожидаемым id/status/heartbeat/progress поколения. Обнулите owner_id и locked_until,
обновите updated_at; при несовпадении условий остановитесь и перечитайте статус.
Не удаляйте поколения, не меняйте active_generation_id и не присваивайте READY вручную.
После сброса проверьте, не стартовал ли RECOVERY; иначе один раз вызовите штатный
POST `/admin/folio/customer-debtors/snapshot/refresh`. При timeout сначала проверьте
статус, не повторяйте запрос вслепую. Новый расчёт сам завершает брошенное поколение.

Подтверждение запуска — новое поколение и рост processedClients/heartbeat.
Подтверждение завершения — новое activeSnapshot и контрольная сверка баланса клиента.
До завершения сохраняется прежний активный снимок. Если новый расчёт завершился
ошибкой, сохраните прежний снимок и диагностируйте причину; старый lease не возвращайте.

Страница должников различает `BUILDING + running=false` и активный расчёт:
показывает «Оновлення перервано», разрешает повторный запуск и сохраняет доступ к
старому отчёту. Опрос продолжается, чтобы заметить автоматический RECOVERY.
При `running=true` кнопка остаётся заблокированной; для старого API без running
сохраняется прежнее поведение. Проверка интерфейса:
`node wp-content/mu-plugins/pc-folio-customer-balance/tests/customer-debtors-status.cjs`.
