...
Проверено по текущему репозиторию и проектным skills: 2026-09-03.

## Сохранение выполненной работы

По решению владельца от 2026-09-21 завершённые и проверенные изменения вместе с
документацией сохраняются коммитом и отправляются в удалённый Git-репозиторий без
повторного запроса разрешения, если для конкретной задачи не указано обратное.
В коммит не включаются посторонние незавершённые изменения или секреты. При
ошибке push нужно сообщить причину и локальный hash: до успешной отправки работа
ещё не сохранена удалённо. Деплой и production-операции требуют отдельного решения;
откат Git не отменяет изменения базы данных. Инструкция для агентов:
[AGENTS.md](../AGENTS.md).
...
| Безопасно продолжить SKU campaign после временной блокировки ФОЛІО | [контракт автоматического повтора](api/FOLIO_ACCOUNTING_PRICE_LOCK_RETRY_BACKEND_TASK.md) |
...
Начинает с [WHOLESALE_CUSTOMER_GUIDE_UK.md](WHOLESALE_CUSTOMER_GUIDE_UK.md): выбирает
режим склада, заказывает через каталог, список или файл, работает с экспортом и
черновиками, повторяет старые заказы, проверяет распределение в корзине, проходит
checkout и находит баланс, заказы и документы ФОЛІО в кабинете.

### Менеджер и оператор

Использует общую пользовательскую инструкцию, отдельную инструкцию оптового клиента,
media guide и операционный runbook. Для
