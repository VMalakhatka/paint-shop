...
### Месячная прибыль Лавки

Проверено: 2026-09-08 по Java `FolioProfitReportService` / `FolioProfitClassifier`,
API-документам `kreul_com_ua/docs/api/FOLIO_PROFIT_REPORT_*.md`, повторному расчёту
сайта за июль и контрольным выгрузкам владельца.

Персональный `$lavka-profit-report` установлен в пользовательском каталоге Codex
и предназначен для WordPress и Java-проекта. В backend-задаче его следует сочетать
с `$work-with-folio-mssql`, для UI — с `$lavka-woo`, для книг — со skill spreadsheets.
Skill хранит матрицу отборов, правила расчёта, колонки сравнения, июльский контроль
и открытые решения. Точный текущий API остаётся в документации Java.

Порядок ревизии: выбрать месяц → дождаться успешного расчёта → загрузить аудит с
теми же параметрами → сравнить реальные документы с выгрузками → отделить
совпадение суммы от правильности периода. Новый лист Одессы добавлен к контрольной
книге с сохранением Киева; личные выгрузки и реквизиты в Git не включаются.

Расширение окна кандидатов всех расходов до минимум M−1/M/M+1, контроль
повреждённых периодов, строгие статьи без периода, дополнительные работы Киева
и дробление сводных строк имеют отдельные статусы в skill; создание skill
не означает, что эти изменения уже внесены в Java или опубликованы на сайте.
Спорный документ хозяйственных расходов за два месяца сохраняется для решения,
а не исправляется автоматически. 08.09.2026 по запросу владельца
[сводное задание](api/FOLIO_PROFIT_REPORT_RECONCILIATION_BACKEND_TASK.md) передано
в «Предварительный анализ проекта» проекта `kreul_com_ua`. Java и фронт обновлены
локально; production-деплой не выполнялся. Детализация и Excel-экспорт описаны
в [операционном runbook](OPERATIONS_RUNBOOK.md#прибыль).

```text
$lavka-profit-report $work-with-folio-mssql
Продолжи ревизию месячной прибыли Лавки. Изучи текущий Java/API и открытые решения
скилла; покажи факты и требуемые изменения до изменения расчёта.
```
