...
## Список ожидания клиентов — первый этап

Список включён как production-пилот 2026-09-12. Фоновый email включён
для этого пилота 2026-09-13; WordPress/MariaDB тесты пройдены, production-транспорт
принял одно явно тестовое письмо. Это не подтверждение доставки во входящие.
Владелец — `pc-order-import-export`: `Waitlist`, `WaitlistStore`, `WaitlistModel`, `WaitlistMail`.
Это новый интерфейс; ранее существовавший список ожидания не подтверждался и,
по уточнению владельца, не был реализован. Наличие общее для клиентов; режим
распределения склада влияет на действие с корзиной, а не на принадлежность stock.

Woo хранит явные запросы и согласие на отслеживание, показывает уведомления в
кабинете. Новая таблица состояния создаётся только явным включением менеджером;
контракт, ограничения, тесты и rollback — в
[README плагина](../wp-content/plugins/pc-order-import-export/README.md#customer-waiting-list--stage-1).
При обработке отслеживаемого черновика HPOS-совместимые order meta сохраняют
исходные строки и отдельный результат добавления. `cart_added` не означает продажу.

ФОЛИО расходная накладная остаётся основным будущим источником потребления клиента.
Email использует отдельное согласие, CAS-журнал перед отправкой и фоновый
опрос общего наличия; неизвестный исход не повторяется автоматически.
Автоматическое погашение по покупкам и статистические рекомендации ещё не реализованы. План и предложения по Java-проекциям —
[план клиентских рекомендаций](CUSTOMER_RECOMMENDATIONS_PLAN.md).
...
Единый поиск каталога соединяет MU `psu-search-filters.php` (форма, категория и
ограничения основного запроса) с `paint-shop-ux/inc/catalog-search.php`
(read-only admin-AJAX подсказки Relevanssi). Фото и наличие берутся из Woo;
персональная цена рассчитывается в текущей сессии без общего кэша ответа.
Подсказки не обращаются к Java и не меняют корзину. Проверено локально 2026-09-24;
контракт, тесты и границы поставки — в [README владельца](../wp-content/plugins/paint-shop-ux/README.md).
...
исправление свободного остатка ввело schema 6. Изменение 2026-09-20 требует
`ANALYTICS_SCHEMA_VERSION=7`, Flyway V16 и новых снимков: Java классифицирует
текущие внутренние резервы, WordPress прибавляет их к свободному остатку
исходного склада без зависимости от получателя. Маршруты и сверка сети для этой
поправки не нужны; отдельный контроль физических транспортных остатков сохранён. Точные запросы и границы потребителя:
...
### Согласие клиента с подготовленным заказом (2026-09-25)

`pc-order-import-export/inc/CustomerApproval.php` владеет запросом и записью
клиентского согласия. Входы: менеджерский Woo-заказ или ACCOUNT ФОЛИО; существующий
прямо связанный Woo-заказ используется как цель. Снимок включает связанные
документы, читаемые существующим customer-documents GET с проверкой владельца.
Согласие и пожелания доставки/оплаты хранятся в приватном CPT отдельно от платежей,
статусов Woo и финансовых документов. Внешних write-вызовов нет. Клиентское подтверждение
не является разрешением кода на отгрузку или фискализацию. Менеджерское руководство,
хранение, конкуренция, ограничения и выпуск —
[Operations runbook](OPERATIONS_RUNBOOK.md#подтверждение-подготовленного-менеджером-заказа).
