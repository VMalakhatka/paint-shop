...
колонок. Можна [завантажити порожній шаблон CSV](examples/wholesale-order-template.csv)
або створити файл самостійно. Найпростіший CSV-файл має такий вигляд:
...
Підтримуються звичні назви колонок:

- товар: `sku`, `Артикул`, `Код товару` або `gtin`, `ean`, `barcode`,
  `Штрихкод`;
- кількість: `qty`, `К-сть`, `Кількість` або `шт`;
- необов'язкова ціна: `price`, `Ціна` або `Вартість`.
...
від'ємною кількістю пропускаються. Якщо у файлі є і штрихкод, і артикул, сайт
спочатку шукає товар за штрихкодом, а потім — за артикулом.
...
## 9. Як повторити чернетку або старе замовлення

Завантажити товари можна не лише з чернетки, а й зі старого замовлення
WooCommerce.

1. Відкрийте [«Мої замовлення»](https://kreul.com.ua/my-account/orders/).
2. Для чернетки натисніть дію **«У кошик»**. Для старого замовлення відкрийте його
   деталі та натисніть **«Завантажити чернетку у кошик»** — така назва кнопки
   використовується і для вже оформлених замовлень.
3. Відкрийте кошик і перевірте, що саме було додано.
4. Перевірте поточні ціни, доступну кількість і рядки **«Списання»**.

> **Важливо:** завантаження чернетки або старого замовлення **спочатку очищає
> поточний кошик**, а потім додає товари з вибраного замовлення. Якщо поточний
> кошик потрібен, спочатку збережіть його в окрему чернетку або вивантажте у файл.

Сайт додає тільки ті товари, які досі існують і доступні за поточними правилами.
Видалені або недоступні позиції можуть бути пропущені; стара ціна не переноситься.
Для старого замовлення, розділеного по складах, уважно відкрийте потрібну частину
та обов'язково перевірте отриманий кошик.
...
| Після завантаження чернетки зникли товари з поточного кошика | Це очікувана поведінка: чернетка або старе замовлення замінює поточний кошик. Відновіть попередній кошик з окремої чернетки чи експортованого файлу, якщо ви їх зберегли. |
