...
Java реализовала многоскладской transit calculationVersion=2; WordPress подключает
его после capability-gate к аналитике, закупочному preview и CSV/XLSX.
Локальные fixtures проверяют источники, сумму, неизвестность и смену поколений.
Остаются деплой Java/WordPress и операторская сверка рабочих SKU/партий.
Нет автоматических ETA, назначения группе или сверки дублей открытых заказов:
менеджер подтверждает их отдельно; ручной ноль без подтверждения не допускает
готовую рекомендацию. Регламент: `OPERATIONS_RUNBOOK.md`, транспортные склады.
